...
Copy-paste friendly wording for presenting GHOST on resumes, LinkedIn, GitHub, and in interviews. These snippets describe the current evidence honestly: ROS 2 target tracking with Raspberry Pi AprilTag hardware validation, static replay artifacts, and no claim of flight test, production deployment, or closed-loop vehicle command.

## Resume Bullets

Short one-line bullet:

- Built a ROS 2 target-tracking prototype using Raspberry Pi AprilTag measurements, comparing formal IMM and heuristic MH trackers with hardware-validated replay artifacts.

Technical bullet:

- Developed a ROS 2 AprilTag target-tracking pipeline with Raspberry Pi camera input, formal IMM and heuristic MH trackers, live-bag plotting, and a static replay dashboard; final hardware bag `live_camera_calibrated_R_01` ran for 48.28 s with 655 vision measurements and near-30 Hz tracker odometry.

Senior/industry-style bullet:

- Owned an end-to-end robotics validation package for intermittent-visibility target tracking, integrating ROS 2 topics, Raspberry Pi AprilTag sensing, IMM/MH estimator comparison, dropout-state telemetry, reproducible plots, and a dependency-free dashboard showing up to 77 IMM prediction-only steps and 2.849 s max measurement age during target loss.

## LinkedIn Project Description

GHOST is a ROS 2 target-tracking project that uses Raspberry Pi AprilTag measurements to compare a formal IMM tracker against a heuristic MH tracker during intermittent visibility. The final calibrated hardware bag (`live_camera_calibrated_R_01`) produced 48.28 s of replayable evidence with 655 vision measurements, camera poses at 13.57 Hz, and tracker odometry near 30 Hz. I packaged the results as a project report, hardware plots, and a fully static replay dashboard so reviewers can inspect measurement streams, tracker estimates, status transitions, and dropout prediction behavior without running ROS.

## GitHub Pinned Repo Description

ROS 2 target-tracking prototype with Raspberry Pi AprilTag hardware validation, formal IMM vs heuristic MH comparison, dropout prediction telemetry, plots, report, and a static replay dashboard.
...
- I compared a formal IMM estimator with a heuristic MH baseline to make tradeoffs visible during live replay and target loss.
...
- The final validation used a calibrated Raspberry Pi AprilTag hardware bag, not only synthetic measurements.
...
Task: Build and package a hardware-validated target-tracking workflow that showed raw AprilTag detections, formal IMM tracking, heuristic MH tracking, and dropout behavior in a form that other engineers could review quickly.
