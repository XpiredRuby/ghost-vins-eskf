...
fetch('/api/plan').then(r=>r.json()).then(p=>{plan=p;document.getElementById('meta').textContent=`#${p.sequence} · ${p.trial_id} · ${p.condition_id}`;post('page_loaded')}).catch(e=>{cue.textContent='PLAN ERROR';instruction.textContent=e.message});
...
function nextPhase(){phaseIndex++;if(phaseIndex>=plan.phases.length){finish();return}const p=plan.phases[phaseIndex];stage.className=`stage phase-${p.phase_type}`;cue.textContent=p.cue;instruction.textContent=p.instruction;phaseStart=performance.now();totalPaused=0;paused=false;speak(p.speak||p.cue);post('phase_started',{cue:p.cue,duration_s:p.duration_s});if(p.duration_s<=0){setTimeout(nextPhase,650);return}tick()}
...
function finish(){cancelAnimationFrame(raf);stage.className='stage phase-done';cue.textContent='DONE';count.textContent='✓';instruction.textContent='Keep all evidence. Verify the measured gap and recorder outputs before marking this trial accepted.';bar.style.width='100%';post('cue_sequence_completed');document.getElementById('pause').disabled=true;speak('Trial cue sequence complete')}
</script></body></html>'''
