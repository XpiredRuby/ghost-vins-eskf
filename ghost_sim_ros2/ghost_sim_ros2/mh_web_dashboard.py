...
<body>
  ...
  <main>
    <section class="left">
      ...
      <div class="card">
        <div class="card-title"><span id="mapTitle">Live relative-weight future map</span><span class="pill" id="mapMode">LIVE</span></div>
    ...
    <section class="card panel">
      ...
      <table>
        <thead><tr><th>#</th><th>model</th><th>Relative weight</th><th>x,y m</th></tr></thead>
      ...
      <div class="math">
        ...
        Top-1 = the highest-weight future.<br>
        Top-3 = whether the correct future stayed inside the best three guesses.<br>
        Relative weights are heuristic ranking values, not calibrated probabilities.<br>
        Replay = recorded relative-weight map and events from the last trial.
      </div>
    </section>
  </main>
<script>
...
function hypWeight(h){ return Number(h.relative_hypothesis_weight ?? h.probability ?? 0); }
...
function drawPath(t,h,color,rank){ const path=h.path||[]; if(path.length<1)return; const weight=Math.max(0.02,Math.min(1,hypWeight(h))); ctx.save(); ctx.strokeStyle=color; ctx.fillStyle=color; ctx.globalAlpha=0.20+0.80*weight; ctx.lineWidth=2+7*weight; ctx.beginPath(); for(let i=0;i<path.length;i++){ const p=toCanvas(t,path[i].x_m,path[i].y_m); if(i===0)ctx.moveTo(p.px,p.py); else ctx.lineTo(p.px,p.py); } ctx.stroke(); const end=toCanvas(t,path[path.length-1].x_m,path[path.length-1].y_m); ctx.beginPath(); ctx.arc(end.px,end.py,5+7*weight,0,Math.PI*2); ctx.fill(); ctx.globalAlpha=1; ctx.font='bold 14px Arial'; ctx.fillText(`${rank}: ${Math.round(weight*100)}%`,end.px+10,end.py-8); ctx.restore(); drawEllipse(t,h,color,0.32); }
...
function renderHypTable(payload){ const hyps=(payload&&payload.hypotheses)||[]; rows.innerHTML=''; for(const h of hyps.slice(0,5)){ const tr=document.createElement('tr'); const weight=hypWeight(h); const p=Math.max(0,Math.min(100,100*weight)); tr.innerHTML=`<td>${h.rank}</td><td class="model">${h.model}</td><td><b>${pct(weight)}</b><div class="bar"><span style="width:${p}%"></span></div></td><td>${fmt(h.x_m,2)}, ${fmt(h.y_m,2)}</td>`; rows.appendChild(tr); } }
