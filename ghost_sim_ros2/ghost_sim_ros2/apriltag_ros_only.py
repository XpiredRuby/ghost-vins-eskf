...
def publish_ros_pose(tvec):
    ...
    msg.header.frame_id = "ghost_floor"
    msg.pose.pose.position.x = cam_z
    msg.pose.pose.position.y = cam_x
...
def draw_axis(frame, rvec, tvec):
    ...
    points = [(int(round(x)), int(round(y))) for x, y in projected]
    origin = points[0]
...
if __name__ == "__main__":
    rclpy.init()
    ROS_NODE = rclpy.create_node("ghost_live_apriltag_pose_viewer")
