...
This is the pre-hardware acceptance harness for the GHOST V1 tracker:
...
class RegimeConfig:
    ...
    stationary_enter_speed_mps: float = 0.08
    stationary_exit_speed_mps: float = 0.14
...
def make_scenarios() -> Dict[str, Scenario]:
    ...
    def false_jump(t: float) -> Tuple[float, float]:
    ...
    return {
        ...
        "false_measurement_jump": Scenario("false_measurement_jump", "visible target with one outlier", 5.0, 99.0, 99.0, false_jump, lambda t: True),
    }


def generate_measurements(scenario: Scenario, config: RegimeConfig, noise_std_m: float = 0.004, seed: int = 42) -> List[Measurement]:
    ...
    for k in range(n + 1):
        ...
        if visible:
            mx = x + rng.gauss(0.0, noise_std_m)
            my = y + rng.gauss(0.0, noise_std_m)
            if scenario.name == "false_measurement_jump" and abs(t - 2.5) < 0.5 * config.dt_s:
                mx += 0.45
                my -= 0.25
...
def score_scenario(scenario: Scenario, measurements: List[Measurement], outputs: List[TrackerOutput]) -> ScenarioMetrics:
    errs = []
    hidden_outputs = []
    top1_terminal_errs = []
    top3_best_errs = []
    ...
    for m, o in zip(measurements, outputs):
        ...
        if not m.visible and o.initialized:
            ...
            if o.hypotheses:
                terminal_truth = scenario.truth_fn(m.t_s + 1.5)
                ...
                if h0.path and o.x_m is not None and o.y_m is not None:
                    stationary_false_motions.append(_err((o.x_m, o.y_m), h0.terminal_xy()) / 1.5)
    ...
    if scenario.name == "stationary_hide_reveal":
        ...
        if first_hidden_probability < 0.90:
            passed = False
            notes.append("stationary_hold prior below 0.90")
        if false_motion > 0.01:
            passed = False
            notes.append("dominant hidden path moved while target was stationary")
        if hold_frac < 0.80:
            passed = False
            notes.append("stationary hold did not cover most hidden samples")
        ...
        if top3 > 0.40:
            passed = False
            notes.append("top3 future too far for stop-behind-wall scenario")
    elif scenario.name == "lateral_hidden_motion":
        if top3 > 0.60:
            passed = False
            notes.append("top3 failed to keep lateral hidden motion plausible")
    elif scenario.name == "long_occlusion_reset":
        if reset_count == 0:
            passed = False
            notes.append("long occlusion did not trigger reset/unknown state")
    elif scenario.name == "false_measurement_jump":
        if rmse > 0.10:
            passed = False
            notes.append("visible RMSE too high after injected outlier")
...
def write_summary(run_dir: Path, metrics: List[ScenarioMetrics]) -> None:
    summary = {
    ...
    lines = [
        ...
        "This report is generated with no Pi, no camera, no ROS runtime, and known synthetic truth.\n",
        f"**Overall:** {'PASS' if summary['overall_pass'] else 'FAIL'}  ",
        f"**Scenarios:** {summary['pass_count']}/{summary['scenario_count']} passing\n",
        "| Scenario | Result | Top hidden model | P(top) | RMSE m | Top-1 future m | Top-3 best m | Stationary false motion m/s | Reset count | Notes |",
        "|---|---:|---|---:|---:|---:|---:|---:|---:|---|",
    ]
    for m in metrics:
        lines.append(
            ...
            f"{m.stationary_false_motion_mps:.4f} | {m.reset_count} | {m.notes} |"
        )
    lines.append("\n## Acceptance Gates\n")
    lines.append("- `stationary_hide_reveal`: top hidden model must be `stationary_hold`, probability >= 0.90, dominant hidden path must not move.")
...
<style>body{{font-family:system-ui,sans-serif;margin:24px;background:#111;color:#eee}}.card{{background:#1c1c1c;border:1px solid #333;border-radius:12px;padding:16px;margin:12px 0}}.pass{{color:#7ddc7d;font-weight:700}}.fail{{color:#ff7777;font-weight:700}}table{{width:100%;border-collapse:collapse}}th,td{{border-bottom:1px solid #333;text-align:left;padding:8px}}code{{color:#9ed0ff}}</style></head><body>
...
def run_acceptance(out_dir: str) -> List[ScenarioMetrics]:
    ...
    for scenario in make_scenarios().values():
        ...
        m = score_scenario(scenario, measurements, outputs)
        metrics.append(m)
